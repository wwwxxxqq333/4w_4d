...
// constructor
MPPI3DCore::MPPI3DCore(param::CommonParam& param_common, param::MPPI3DParam& param)
{
    // save parameters
    ...
    XDIM = target_system_mppi_3d::DIM_STATE_SPACE;
    UDIM = target_system_mppi_3d::DIM_CONTROL_SPACE;
...
// mppi solver
common_type::VxVyOmega MPPI3DCore::solveMPPI(
...
)
{
    ...
    {
        // initialize state
        State x = target_system_mppi_3d::convertXYYawToStateSpace3D(observed_state); // in mppi_3d, StateSpace3D is equal to XYYaw
        ...
        for (int t = 1; t < T+1; t++)
        {
            ...
            costs_[k] += controller_mppi_3d::stage_cost(
                    x,
                    u_samples_[k][t-1],
                    prev_control_input,
                    collision_costmap,
        ...
        // add terminal cost
        costs_[k] += controller_mppi_3d::terminal_cost(x, goal_state, param_);
    }
    ...
    for (int t = 1; t < T; t++)
    {
        ...
        state_cost_ += controller_mppi_3d::stage_cost(
                x_opt_seq_[t],
                u_opt_seq[t-1],
                prev_control_input,
                collision_costmap,
    ...
    // add terminal cost
    state_cost_ += controller_mppi_3d::terminal_cost(x_opt_seq_[T-1], goal_state, param_);

    // convert optimal control command to VxVyOmega
    common_type::VxVyOmega optimal_vxvyw_cmd = target_system_mppi_3d::convertControlSpace3DToVxVyOmega(u_opt_seq[0]);
    optimal_vxvyw_cmd.clamp();
...
ControlSeqSamples MPPI3DCore::generateNoiseMatrix(ControlSeq& sigma)
{
...
// calculate weight for each sample 
Samples MPPI3DCore::calcWeightsOfSamples(const Samples& costs)
{
    // initialize weights
...
void MPPI3DCore::initSaviskyGolayFilter(
...
Eigen::MatrixXd MPPI3DCore::calcSaviskyGolayCoeffs(
    const int half_window_size, 
    const unsigned int poly_order, 
    const double delta
)
...
// apply savisky-golay filter to get smoothed u_opt_seq[0]
Control MPPI3DCore::applySaviskyGolayFilter(ControlSeq& u_opt_seq)
{
...
// get calc time [ms]
float MPPI3DCore::getCalcTime()
{
    return calc_time_;
}

// get state cost of the latest optimal trajectory
double MPPI3DCore::getStateCost()
{
    return state_cost_;
}

// get controller name
std::string MPPI3DCore::getControllerName()
{
    return param_.controller.name;
}

// check if the vehicle is reached to the goal
bool MPPI3DCore::isGoalReached()
{
    return is_goal_reached_;
}

// get optimal vehicle command (8DoF)
common_type::VehicleCommand8D MPPI3DCore::getOptimalVehicleCommand()
{
    return target_system_mppi_3d::convertControlSpace3DToControlSpace8D(u_opt_latest_, param_);
}

// return optimal state sequence
std::vector<common_type::XYYaw> MPPI3DCore::getOptimalTrajectory()
{
    std::vector<common_type::XYYaw> optimal_state_sequence;
    for (int t = 0; t < T; t++)
    {
        common_type::XYYaw state = target_system_mppi_3d::convertStateSpace3DToXYYaw(x_opt_seq_[t]);
...
StateSeqSamples MPPI3DCore::getFullSampledTrajectories()
{
    ...
    for (int k = 0; k < K; k++)
    {
        ...
        for (int t = 0; t < T; t++)
        {
            common_type::XYYaw state = target_system_mppi_3d::convertStateSpace3DToXYYaw(x_samples_[k][t]);
...
StateSeqSamples MPPI3DCore::getEliteSampledTrajectories(int elite_sample_size)
{
    ...
    for (int k = 0; k < elite_sample_size; k++)
    {
        ...
        for (int t = 0; t < T; t++)
        {
            common_type::XYYaw state = target_system_mppi_3d::convertStateSpace3DToXYYaw(x_samples_[costs_rank_[k]][t]);
            state_sequence.push_back(state);
        }
...
std::vector<common_type::VxVyOmega> MPPI3DCore::getOptimalVxVyOmegaSequence()
{
    std::vector<common_type::VxVyOmega> optimal_control_input_sequence;
    for (int t = 0; t < T; t++)
    {
        common_type::VxVyOmega control_input = target_system_mppi_3d::convertControlSpace3DToVxVyOmega(u_opt_seq_latest_[t]);
        optimal_control_input_sequence.push_back(control_input);
    }
    return optimal_control_input_sequence;
}

// set optimal control sequence
void MPPI3DCore::setOptimalVxVyOmegaSequence(std::vector<common_type::VxVyOmega>& u_opt_seq)
{
    // save optimal control sequence
    for (int t = 0; t < T; t++)
    {
        u_opt_seq_latest_[t] = target_system_mppi_3d::convertVxVyOmegaToControlSpace3D(u_opt_seq[t]);
    }

    // apply savisky-golay filter and get u_opt_latest_
    if (param_.controller.use_sg_filter)
    {
